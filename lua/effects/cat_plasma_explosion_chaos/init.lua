...
function EFFECT:Init(data)
	...
	for i = 0, (NumParticles / 5) do
		...
		if (particle) then
			...
			particle:SetStartSize(math.Clamp(50 * vScale, 5, 100)) -- Limits explosion size
			particle:SetEndSize(math.Clamp(25, 5, 50))
	...
	for i = 0, (NumParticles) do
		...
		if (particle) then
			...
			particle:SetStartAlpha(math.Rand(200, 255))
			particle:SetEndAlpha(0)
			particle:SetStartSize(math.Clamp(30 * vScale, 2, 20)) -- Ensures a reasonable size
			particle:SetEndSize(0)
			particle:SetRoll(math.Rand(0, 360))
			particle:SetRollDelta(math.Rand(-2, 2))
			particle:SetColor(55, 15, 255)
		end
	end
	...
	for i = 0, (NumParticles * 2) do
		...
		if (particle) then
			...
			particle:SetEndAlpha(0)
			particle:SetStartSize(math.Clamp(35 * vScale, 5, 75)) -- Prevents oversized particles
			particle:SetEndSize(0)
			particle:SetRoll(math.Rand(0, 360))
			particle:SetRollDelta(math.Rand(-2, 2))
			local Col = math.Rand(10, 75)
			particle:SetColor(55, 15, 255 - Col)
		end
	end
