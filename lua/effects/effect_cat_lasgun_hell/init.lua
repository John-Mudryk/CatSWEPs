...
local Mat_Impact = Material("decals/dark")
local Mat_Beam = Material("entities/effect_cat_laser_hot.png")
local Mat_TracePart = Material("sprites/physg_glow2")
...
local beamcol = Color(255, 255, 255, 255)
local beamcol2 = Color(255, 165, 0, 255)
...
function EFFECT:Render()
	...
	render.DrawQuadEasy(tracerpos, self.Normal, 6, 6, beamcol2, self.rot - 60)
	tracerpos = Lerp(math.Clamp(self.LifeTime / self.Life - 0.05, 0, 1), endPos, startPos)
	render.DrawQuadEasy(tracerpos, self.Normal, 6, 6, beamcol2, self.rot - 30)
	tracerpos = Lerp(math.Clamp(self.LifeTime / self.Life, 0, 1), endPos, startPos)
	render.DrawQuadEasy(tracerpos, self.Normal, 6, 6, beamcol2, self.rot)
	tracerpos = Lerp(math.Clamp(self.LifeTime / self.Life + 0.05, 0, 1), endPos, startPos)
	render.DrawQuadEasy(tracerpos, self.Normal, 6, 6, beamcol2, self.rot + 30)
	tracerpos = Lerp(math.Clamp(self.LifeTime / self.Life + 0.1, 0, 1), endPos, startPos)
	render.DrawQuadEasy(tracerpos, self.Normal, 6, 6, beamcol2, self.rot + 60)
	...
	render.SetMaterial( self.Mat )
		render.DrawBeam( self.StartPos, 										// Start
					 ...
					 Color( 255, 50, 0, math.Clamp(self.Alpha, 0,255)) )		// Color (optional)'
end
