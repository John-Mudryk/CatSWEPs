...
local skin_names = {
	[0] = "Crimson Regalia (World Eaters)",
	[1] = "Azure Phantasm (Alpha Legion)",
	[2] = "Royal Terrorplate (Night Lords)",
	[3] = "Obsidian Steel (Iron Hands)",
	[4] = "Dark Sigil Trim (First Legion)",
	[5] = "Spectral Aurum (Star Hunters)",
	[6] = "Pale Deathsheen (Death Guard)",
	[7] = "Radiant Purity (Imperial Heralds)",
	[8] = "Hazard Alloy Mk.IV (Iron Warriors)",
	[9] = "Pallid Iron (War-Born)",
	[10] = "Prospero Red (Thousand Sons)",
	[11] = "Scripture-Forged Iron (Word Bearers)",
	[12] = "Frosted Bronze (Space Wolves)",
	[13] = "Verdant Embersteel (Salamanders)",
	[14] = "Solar Sigilite (Imperial Fists)",
	[15] = "Serpentine Argent (Sons of Horus)",
	[16] = "Battleworn Bronze (War Hounds)",
	[17] = "Khan's Crest (White Scars)",
	[18] = "Lunar Eclipse Plate (Luna Wolves)",
}
...
local function ApplySkinToElements(wep, skin, color)
	...
	for _, name in ipairs(target_elements) do
		if wep.VElements and wep.VElements[name] then
			...
			if IsValid(elem.modelEnt) then
				elem.modelEnt:SetColor(color)
				elem.modelEnt:SetRenderMode(RENDERMODE_TRANSALPHA)
			end
		end

		if wep.WElements and wep.WElements[name] then
			local elem = wep.WElements[name]
			elem.skin = skin
			elem.color = color
	...
	-- We skip skin SetSkin here due to it not working live
	-- The actual skin will apply properly on next draw or deploy
...
-- Called when attachment is applied
...
-- Detach cleanup and save
function ATTACHMENT:Detach(wep)
	if not IsValid(wep) then return end

	local ply = wep:GetOwner()
	if IsValid(ply) then
		if wep.PowerfistSkinIndex then
			ply:SetNWInt("PowerfistSkin_" .. wep:GetClass(), wep.PowerfistSkinIndex)
		end
		if wep.PowerfistColor then
