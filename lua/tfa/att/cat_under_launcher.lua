...
ATTACHMENT.Ent = "cat_exp_glauncher"
ATTACHMENT.Damage = 450
			...
			if CurTime() >  myself:GetNextPrimaryFire() and TFA.Enum.ReadyStatus[ myself:GetStatus() ] and not myself:GetSprinting() then
				...
				if myself:Clip2() > 0 then
					if SERVER then
						...
						ent:SetPos( myself.Owner:GetShootPos() + (myself.Owner:EyeAngles():Forward()*15) + (myself.Owner:EyeAngles():Right()*5) + (myself.Owner:EyeAngles():Up()*(-10)) )
						ent:SetAngles( myself.Owner:EyeAngles() )
						ent:SetModel( myself:GetStat("Primary.ProjectileModel") or self.DefaultModel )

                    				local primaryDamage = myself:GetStat("Primary.Damage") or 100
						ent.Damage = primaryDamage * 4
