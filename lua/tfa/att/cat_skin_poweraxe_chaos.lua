...
local axe_config = {
	["cat_chaos_legacy_poweraxe"] = {
		element = "poweraxeCH",
		blade = table.Range(0, 4),
		backblade = table.Range(0, 6),
		decor = table.Range(0, 6),
	},
	["cat_chaos_legacy_powerbattleaxe"] = {
		element = "poweraxeCHB",
		blade = table.Range(0, 4),
		backblade = nil,
		decor = table.Range(0, 4),
	}
}
...
local function ApplyBodygroups(wep, elem_name, blade, backblade, decor)
	...
	for _, elem in pairs({v_elem, w_elem}) do
		if elem then
			...
			if blade then elem.bodygroup[2] = blade end
			if backblade ~= nil then elem.bodygroup[3] = backblade end
			if decor then elem.bodygroup[1] = decor end
		end
	end
	...
	timer.Simple(0, function()
		for _, elem in pairs({v_elem, w_elem}) do
			if IsValid(elem and elem.modelEnt) then
				...
				if backblade ~= nil then elem.modelEnt:SetBodygroup(2, backblade) end
...
local function OpenAxeForgeMenu(wep)
	...
	local backblade
		if config.backblade then
			backblade = wep.SelectedBack or config.backblade[1] or 0
		else
			backblade = nil
	end
	...
	local function CreateScrollList(parent, title, options, selected, update, x, class)
		if not options then return end -- Skip if options not defined (e.g. backblade = nil)
		...
		for _, id in ipairs(options) do
			...
			if title == "Blade" then
				name = blade_names[id] or ("Blade " .. id)
			elseif title == "Back Blade" then
				name = backblade_names[id] or ("Back " .. id)
			elseif title == "Decoration" then
				name = (decor_names[class] and decor_names[class][id]) or ("Decor " .. id)
	...
	CreateScrollList(frame, "Blade", config.blade, function() return blade end, function(v) blade = v end, 20, class)
	CreateScrollList(frame, "Back Blade", config.backblade, function() return backblade end, function(v) backblade = v end, 230, class)
	CreateScrollList(frame, "Decoration", config.decor, function() return decor end, function(v) decor = v end, 440, class)
...
function ATTACHMENT:Attach(wep)
	...
	local back
		if config.backblade then
			back = ply:GetNWInt("Axe_Back_" .. class, config.backblade[1] or 0)
		else
			back = nil
	end
	...
	hook.Add("HUDPaint", GetHookID(wep) .. "_HUD", function()
		...
		local decor_display = (decor_names[class] and decor_names[class][decor]) or ("Decor " .. tostring(decor))
		draw.SimpleTextOutlined(
			"Decor: " .. decor_display,
			"DermaLarge",
			ScrW() / 2,
