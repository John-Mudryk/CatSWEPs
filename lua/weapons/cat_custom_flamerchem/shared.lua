...
SWEP.Category				= "Cat's SWEPs" --The category.  Please, just choose something generic or something I've already done if you plan on only doing like one swep..
...
SWEP.PrintName				= "Flamer Chemical"		-- Weapon name (Shown on HUD)
...
PrecacheParticleSystem("cat_flamethrower_halloween")
...
function SWEP:Think2( ... )
	...
	if shooting ~= self.Shooting_Old then
		if shooting then
			...
			fx:SetEntity(self)
			fx:SetAttachment(1)
			util.Effect("cat_flame_green",fx)
			if self:IsFirstPerson() then
				ParticleEffectAttach("cat_flamethrower_halloween",PATTACH_POINT_FOLLOW,self.OwnerViewModel,1)
			else
				ParticleEffectAttach("cat_flamethrower_halloween",PATTACH_POINT_FOLLOW,self,1)
			end
		else
...
SWEP.Type                       = nil 
SWEP.Type_Displayed             = "Magnus Pattern Mk. II Gorgon"