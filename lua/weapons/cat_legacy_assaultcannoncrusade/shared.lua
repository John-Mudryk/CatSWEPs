...
SWEP.PrintName				= "Crusade Rotary Cannon"		-- Weapon name (Shown on HUD)	
...
SWEP.ViewModel				= "models/joazzz/weapons/v_heavybolter_crusade.mdl"	-- Weapon view model
...
SWEP.ViewModelBoneMods = {
	["ValveBiped.Anim_Attachment_RH"] = { scale = Vector(0.009, 0.009, 0.009), pos = Vector(0, 0, 0), angle = Angle(0, 0, 0) }
}


SWEP.Primary.Sound			= "weapons/phobosboltgun/fire2.mp3"		-- script that calls the primary fire sound
SWEP.Primary.RPM			= 400		-- This is in Rounds Per Minute This is in Rounds Per Minute / RPM
...
SWEP.Primary.Spread		= 0.002	-- Define from-the-hip accuracy 1 is terrible, .0001 is exact)
...
SWEP.IronSightsPos = Vector(-7.22, -8.006, -6.323)
SWEP.IronSightsAng = Vector(5.743, -0.209, 0)
...
SWEP.Type_Displayed             = "Nomus Pattern Mk. I" 

-- Attachments
SWEP.VElements = {
	["element_name"] = { type = "Model", model = "models/joazzz/weapons/heresy_rotorcannon.mdl", bone = "ValveBiped.Bip01_R_Hand", rel = "", pos = Vector(-26.206, 15.548, 26.858), angle = Angle(0, -8.987, 180), size = Vector(1, 1, 1), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {} }
}

SWEP.WElements = {
	["rotary"] = { type = "Model", model = "models/joazzz/weapons/terminator_assaultcannon.mdl", bone = "ValveBiped.Bip01_R_Hand", rel = "", pos = Vector(37.244, 2.915, -2.599), angle = Angle(-10.988, 0, 180), size = Vector(0.63, 0.63, 0.63), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {} },
	["element_name"] = { type = "Model", model = "models/joazzz/weapons/v_heavybolter_crusade.mdl", bone = "ValveBiped.Bip01_R_Hand", rel = "", pos = Vector(-21.247, 11.262, -8.129), angle = Angle(-8.046, 0, 180), size = Vector(1.25, 1.25, 1.25), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {} }
}
...
SWEP.EventTable = {
    [ACT_VM_RELOAD] = { -- Use the appropriate reload animation activity
        ...
        {time = 1, type = "sound", value = "40k/b_magin.wav"}  -- Magazine in sound
    }
}
...
SWEP.Primary.RPM_MAX 				= 2100

function SWEP:Initialize(...)
    self.StatCache_Blacklist["Primary.RPM"] = true
    self.ReleaseTime = 0 -- Initialize release time
    return BaseClass.Initialize(self, ...)
end

function SWEP:PrePrimaryAttack(...)
    if self.Owner:KeyDown(IN_ATTACK) then
        self.Primary_TFA.RPM = math.Approach(self.Primary_TFA.RPM, self.Primary.RPM_MAX, 60)
    end
...
function SWEP:Think(...)
    if self.Owner:KeyReleased(IN_ATTACK) then
        self.ReleaseTime = CurTime() -- Store the time when the trigger was released
    end

    -- Check if 3 seconds have passed since the trigger was released (didn't work right, set back to 0)
    if self.ReleaseTime > 0 and CurTime() >= self.ReleaseTime + 0 then
        self.Primary_TFA.RPM = 400
        self.ReleaseTime = 0 -- Reset release time
