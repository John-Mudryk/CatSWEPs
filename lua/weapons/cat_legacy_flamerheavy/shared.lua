...
SWEP.Category				= "Cat's Legacy SWEPs" --The category.  Please, just choose something generic or something I've already done if you plan on only doing like one swep..
...
--Range Related
SWEP.Primary.Range = 950 --Flame Distance
...
--[[VIEWMODEL]]--
SWEP.ViewModel = "models/muschi/weapons/muschi_swep_flamer_v.mdl"
SWEP.WorldModel = "models/muschi/weapons/muschi_swep_flamer.mdl"
...
SWEP.VElements = {
	["element_name"] = { type = "Model", model = "models/joazzz/weapons/terminator_heavyflamer.mdl", bone = "weapon_bone", rel = "", pos = Vector(-18.762, 14.673, 2.94), angle = Angle(89.295, -89.897, -0.489), size = Vector(0.5, 0.5, 0.5), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {} }
}

SWEP.WElements = {
	["element_name"] = { type = "Model", model = "models/joazzz/weapons/terminator_heavyflamer.mdl", bone = "ValveBiped.Bip01_R_Hand", rel = "", pos = Vector(-21.229, -24.937, 6.362), angle = Angle(-108.982, 0, 90), size = Vector(1, 1, 1), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {[1] = 1} }
}
...
SWEP.ViewModelBoneMods = {
	["ValveBiped.Bip01_R_Forearm"] = { scale = Vector(1.254, 1.254, 1.254), pos = Vector(3.286, -4.873, -6.093), angle = Angle(0, 0, 0) },
...
game.AddParticles("particles/cat_flamer.pcf")
PrecacheParticleSystem("cat_flamethrower_crit_red")
...
function SWEP:Think2( ... )
	...
	if shooting ~= self.Shooting_Old then
		if shooting then
			self:EmitSound("waw_flamer/fire_in.wav")
			self.NextIdleSound = CurTime() + 0.2
			local fx = EffectData()
			fx:SetEntity(self)
			fx:SetAttachment(1)
			util.Effect("cat_flame_red",fx)
			if self:IsFirstPerson() then
				ParticleEffectAttach("cat_flamethrower_crit_red",PATTACH_POINT_FOLLOW,self.OwnerViewModel,1)
			else
				ParticleEffectAttach("cat_flamethrower_crit_red",PATTACH_POINT_FOLLOW,self,1)
			end
		else
	...
	if shooting then
		if self.NextIdleSound and CurTime() > self.NextIdleSound then
			self:EmitSound("waw_flamer/fire_loop.wav")
			self.NextIdleSound = CurTime() + SoundDuration( "waw_flamer/fire_loop.wav" ) - 0.1
		end
	end
...
SWEP.Type                       = nil 
SWEP.Type_Displayed             = "Locke Pattern Mk. IIIa"