...
SWEP.Category				= "Cat's Legacy Chaos SWEPs" --The category.  Please, just choose something generic or something I've already done if you plan on only doing like one swep..
...
SWEP.PrintName				= "Chaos Flamer"		-- Weapon name (Shown on HUD)
...
--Range Related
SWEP.Primary.Range = 700 --Flame Distance
...
SWEP.VElements = {
	["element_name"] = { type = "Model", model = "models/joazzz/weapons/chaos/flamer.mdl", bone = "weapon_bone", rel = "", pos = Vector(0.814, 0.252, -1.759), angle = Angle(5.018, 0, 0), size = Vector(0.75, 0.75, 0.75), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 1, bodygroup = {[1] = 2, [2] = 1} }
}
SWEP.WElements = {
	["element_name"] = { type = "Model", model = "models/joazzz/weapons/chaos/flamer.mdl", bone = "ValveBiped.Bip01_R_Hand", rel = "", pos = Vector(13.232, 1.095, -3.968), angle = Angle(-12.252, 0, 180), size = Vector(1, 1, 1), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 1, bodygroup = {[1] = 2, [2] = 1} }
}
...
PrecacheParticleSystem("cat_flamethrower")
	...
	if shooting ~= self.Shooting_Old then
		if shooting then
			...
			if self:IsFirstPerson() then
				ParticleEffectAttach("cat_flamethrower",PATTACH_POINT_FOLLOW,self.OwnerViewModel,1)
			else
				ParticleEffectAttach("cat_flamethrower_crit_blue",PATTACH_POINT_FOLLOW,self,1)
			end
		else
...
SWEP.Type                       = nil 
SWEP.Type_Displayed             = "Mk. IIIa HERETIC"