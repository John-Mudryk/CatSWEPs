...
SWEP.PrintName				= "Chaos Legion Plasma Gun"		-- Weapon name (Shown on HUD)	
...
SWEP.ViewModelBoneMods = {
	...
	["mag_bone"] = { scale = Vector(0.083, 0.083, 0.083), pos = Vector(0, 0, 0), angle = Angle(0, 0, 0) }
}
...
SWEP.IronSightsPos = Vector(-4.361, 0, 0.36)
SWEP.IronSightsAng = Vector(-1.999, -1.934, 0)
...
-- Attachments
SWEP.VElements = {
	["element_name"] = { type = "Model", model = "models/joazzz/weapons/chaos/plasmagun.mdl", bone = "weapon_bone", rel = "", pos = Vector(-7.178, 0.347, 0.508), angle = Angle(2.674, 0, 0), size = Vector(0.75, 0.75, 0.75), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {} }
}

SWEP.WElements = {
	["element_name"] = { type = "Model", model = "models/joazzz/weapons/chaos/plasmagun.mdl", bone = "ValveBiped.Bip01_R_Hand", rel = "", pos = Vector(6.992, 1.169, -4.56), angle = Angle(-12.898, 0, 180), size = Vector(1, 1, 1), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {} }
}
	...
	if SERVER then
		...
		if venting then
			if CurTime() >= (self.NextVentingSteam or 0) then
				local ed = EffectData()
				ed:SetEntity(self)
				util.Effect("plasma_venting_chaos", ed)
