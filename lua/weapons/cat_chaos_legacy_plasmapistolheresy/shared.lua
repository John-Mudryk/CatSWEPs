...
SWEP.PrintName				= "Chaos Heresy Plasma Pistol"		-- Weapon name (Shown on HUD)	
...
SWEP.ViewModel				= "models/muschi/weapons/muschi_swep_plasmapistol_v.mdl"	-- Weapon view model
...
SWEP.IronSightsPos = Vector(-3.6, 0, -1.56)
SWEP.IronSightsAng = Vector(-1.3, -0.072, 0)
...
-- Tracer Stuff
SWEP.TracerName		 = "tracer_laser_plasma_chaos"   -- Change to a string of your tracer name.  Can be custom. There is a nice example at https://github.com/garrynewman/garrysmod/blob/master/garrysmod/gamemodes/base/entities/effects/tooltracer.lua
...
SWEP.Type                       = "Pistol" 
SWEP.Type_Displayed             = "Ryza Pattern Mk. II Sunspite"
...
-- Attachments
SWEP.VElements = {
	["plasmapistolCHH"] = { type = "Model", model = "models/joazzz/weapons/plasmapistol_heresy.mdl", bone = "tag_weapon", rel = "", pos = Vector(5.071, 0.256, -0.385), angle = Angle(0, 180, 0), size = Vector(1, 1, 1), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {[0] = 1} }
}

SWEP.WElements = {
	["plasmapistolCHH"] = { type = "Model", model = "models/joazzz/weapons/plasmapistol_heresy.mdl", bone = "ValveBiped.Bip01_R_Hand", rel = "", pos = Vector(7.738, 1.822, -1.371), angle = Angle(164.289, 0, 0), size = Vector(1, 1, 1), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {[0] = 1} }
}
...
SWEP.Attachments = {
    	[1] = { offset = { 0, 0 }, atts = { "cat_am_plasma_fast_chaos", "cat_am_plasma_overcharge_chaos", "cat_am_plasma_bolt_chaos"}, order = 1 },
	...
	if SERVER then
		...
		if venting then
			if CurTime() >= (self.NextVentingSteam or 0) then
				local ed = EffectData()
				ed:SetEntity(self)
				util.Effect("plasma_venting_pistol_chaos", ed)
