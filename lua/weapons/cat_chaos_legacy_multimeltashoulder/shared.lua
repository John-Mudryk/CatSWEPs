...
SWEP.PrintName				= "Chaos Shoulder Multimelta"		-- Weapon name (Shown on HUD)	
...
-- Tracer Stuff
SWEP.TracerName		 = "tracer_laser_melta_chaos"   -- Change to a string of your tracer name.  Can be custom. There is a nice example at https://github.com/garrynewman/garrysmod/blob/master/garrysmod/gamemodes/base/entities/effects/tooltracer.lua
...
function SWEP:ShootEffects()
    -- Call the base class shoot effects to keep default behavior
    self:SendWeaponAnim(ACT_VM_PRIMARYATTACK)
    self.Owner:MuzzleFlash()
    self.Owner:SetAnimation(PLAYER_ATTACK1)

    -- Only play the effect ONCE per shot
    if CLIENT then
        local effect = EffectData()
        effect:SetOrigin(self.Owner:GetShootPos() + self.Owner:EyeAngles():Forward() * 20)
        effect:SetEntity(self)
        util.Effect("swep_flamethrower_flame_chaos", effect) -- Directly specify the effect name
    end
end
